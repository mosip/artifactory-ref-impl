...
DEFAULT_ZIP_PATH=artifactory/libs-release-local/demosdk/default/demosdk.zip
[ -z "$demosdk_zip_file_path" ] && zip_path="$DEFAULT_ZIP_PATH" || zip_path="$demosdk_zip_file_path"

echo "Download the demosdk from $artifactory_url_env"
wget -q --show-progress "$artifactory_url_env/$zip_path"
echo "Downloaded $artifactory_url_env/$zip_path"

FILE_NAME=${zip_path##*/}

DIR_NAME=$demosdk_local_dir_name
...
cd ./$DIR_NAME && chmod +x install.sh && sudo -i | ./install.sh
